...
export default function Staff() {
    ...
    const login = (body) => {
        return rawInstance.get('https://51.20.20.105/api/v1/sites/', body, { withCredentials: true });
    ...
    return (
        <>
            {!loading ? <>
                {/* {contextHolder} */}
                <Divider className="bg-slate-400 before:text-black"><h1><b className='text-4xl'>Site Details</b></h1></Divider>
                <div className='flex py-4 px-4'>
                    <div className='w-8/12 border-slate-700 border-2 rounded-xl p-1'>
                        <List
                            header={<div className='flex justify-between'><b>Site List</b><p className="text-center text-xl dark:bg-slate-700 bg-slate-200 text-gray-800 dark:text-white w-44 hover:bg-gray-50 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg  hover:bg-gray-700 hover:text-gray-50 focus:outline-none dark:focus:ring-gray-800 cursor-pointer" onClick={() => router.push('/site/addSite')}>+ Add Site</p></div>}
                            bordered
                            dataSource={data}
                            renderItem={(item) => (
                                <List.Item onClick={() => setSiteId(item.id)} className='cursor-pointer'>
                                    {item.name}
                                </List.Item>
                            )}
                        />
                    ...
                    <div className="mx-5 w-3/4 border-slate-700 border-2 rounded-xl p-1">
                        <Tabs defaultActiveKey="tab1" type="card">
                            <Tabs.TabPane tab="Site Details" key="tab1">
                                <Step1 setSiteId={setSiteId} siteId={siteId} />
                            </Tabs.TabPane>

                            <Tabs.TabPane tab="Contacts" key="tab2" >
                                <Step2 siteId={siteId} />
                            </Tabs.TabPane>

                            <Tabs.TabPane tab="Preferred staff" key="tab3" >
                                <Step3 siteId={siteId} />
                            </Tabs.TabPane>

                            <Tabs.TabPane tab="Notes" key="tab4" >
                                <Step4 siteId={siteId} />
                            </Tabs.TabPane>

                            <Tabs.TabPane tab="Incident Report" key="tab5" >
                                <Step5 siteId={siteId} />
                            </Tabs.TabPane>

                            <Tabs.TabPane tab="Site Document" key="tab6" >
                                <Step6 siteId={siteId} />
                            </Tabs.TabPane>
                            <Tabs.TabPane tab="Site Rate" key="tab7" >
                                <Step7 siteId={siteId} />
                            </Tabs.TabPane>
                        </Tabs>
                    </div>
